  ...
  useEffect(() => {
    const handleScroll = () => {
      if (window.innerHeight + window.scrollY >= document.documentElement.scrollHeight) {
        setVisibleColleges(prevVisibleColleges => Math.min(prevVisibleColleges + 10, colleges.length));
      }
    };
            ...
            <tr key={index}>
              ...
              <td>
                <div className="college-info">
                  <img src={college.logo_path} alt={`${college.name} Logo`} className="college-logo"/>
                  <div>
                    <h4>{college.name}</h4>
                    <p>{college.location} | {college.approval}</p>
                    <p><b>{college.course}</b></p>
                    <p>JEE-Advanced 2023 Cutoff: {college.cutoff}</p>
                    <div className="action-links">
                      <a href="#">Apply Now</a> | <a href="#">Download Brochure</a>
                    </div>
